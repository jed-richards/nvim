...
local function get_nix_typescript_server_path()
	...
	tsc_bin_path = vim.fn.trim(vim.fn.system("readlink -f " .. vim.fn.shellescape(tsc_bin_path)))
	...
	before_init = function(_, config)
		if vim.tbl_get(config.init_options, "typescript") and not config.init_options.typescript.tsdk then
			...
			if not vim.env.IN_NIX_SHELL then
				config.init_options.typescript["tsdk"] = get_node_modules_typescript_server_path(config.root_dir)
			else
				config.init_options.typescript["tsdk"] = get_nix_typescript_server_path()
			end
		end
	end,
}
